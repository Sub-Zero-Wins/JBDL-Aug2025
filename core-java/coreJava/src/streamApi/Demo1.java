...
public class Demo1 
{
	public static void main(String[] args)
	{
		...
		marks.stream().map(f).filter(p).distinct().forEach(i->System.out.println(i));
		System.out.println(marks.stream().map(f).filter(p).count());
		System.out.println(marks.stream().map(f).filter(p));;
		...
		List<String> l =nestedList.stream().flatMap(list->list.stream()).collect(Collectors.toList());
